...
secureAccount = function () {
  ...
  if (sname == "" || spass == "" || semail == "") {
    ...
    if (waitSecure == 0) {
      waitSecure = 1;
      $.post(
        "system/action/action_secure.php",
        {
          secure_name: sname,
...
setUserTheme = function (item) {
  ...
  $.ajax({
    ...
    success: function (response) {
      $("#actual_theme").attr(
        "href",
        "css/themes/" + response.theme + "/" + response.theme + ".css" + bbfv
      );
...
acceptFriend = function (t, friend) {
  $.post(
    "system/action/action_friend.php",
    {
      add_friend: friend,
    },
    function (response) {
      $(t).parent().remove();
      if (
        $(".friend_request").length < 1 &&
        $("#friends_menu:visible").length
      ) {
...
changeUsername = function () {
  $.post("system/box/edit_name.php", {}, function (response) {
    overModal(response);
  });
};
changeInfo = function () {
...
changeShared = function () {
  $.post("system/box/edit_shared.php", {}, function (response) {
    overModal(response);
  });
};
changeAbout = function () {
  $.post("system/box/edit_about.php", {}, function (response) {
    overModal(response, 500);
  });
};
...
getSoundSetting = function () {
  $.post("system/box/sound.php", {}, function (response) {
...
changeMood = function () {
  $.post("system/box/edit_mood.php", {}, function (response) {
    overModal(response);
  });
};
...
saveUserColor = function (u) {
  $.post(
    "system/action/action_users.php",
    {
...
openAddRoom = function () {
  $.post("system/box/create_room.php", {}, function (response) {
    overModal(response);
  });
};
openShareGold = function (id) {
  $.post(
    "system/box/gold_share.php",
    {
      target: id,
    },
    function (response) {
...
appLeftMenu = function (aIcon, aText, aCall, optMenu) {
  ...
  qmenu +=
    '<div class="fmenu_icon"><i class="fa fa-' + aIcon + ' menui"></i></div>';
...
appPrivInputMenu = function (mIcon, mCall) {
  var privInpMenu =
    '<div class="psub_options" onclick="' +
    mCall +
    '"><img src="' +
    mIcon +
    '"/></div>';
...
noDataTemplate = function () {
  return (
    '<div class="pad_box"><p class="centered_element text_med sub_text">' +
    system.noResult +
    "</p></div>"
  );
};
...
uploadIcon = function (target, type) {
  ...
  if (type == 2) {
    $("#" + target)
      .removeClass("fa-circle-o-notch fa-spin fa-fw")
      .addClass(upIcon);
  } else {
    $("#" + target)
      .removeClass(upIcon)
      .addClass("fa-circle-o-notch fa-spin fa-fw");
  }
};
...
removeFile = function (target) {
  postIcon(2);
  $.post(
    "system/action/action_files.php",
    {
