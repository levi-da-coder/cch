...
warningBox = function (content) {
  var bbox =
    '<div class="pad_box centered_element"><i class="fa fa-exclamation-triangle warn text_ultra bmargin10"></i><h3>' +
    content +
    "</h3></div>";
  showModal(bbox);
};
...
getNewsOptions = function () {
  $.post(
    "system/box/news_post_options.php",
    {
...
getWallOptions = function () {
  $.post(
    "system/box/wall_post_options.php",
    {
...
showPrivateReport = function (id, item) {
  var post_id = id;
  $.post(
    "system/box/show_private_report.php",
    {
      private_report: id,
    },
    function (response) {
...
showChatReport = function (id, item) {
  var post_id = id;
  $.post(
    "system/box/show_chat_report.php",
    {
      chat_report: id,
    },
    function (response) {
...
showWallReport = function (id, item) {
  var post_id = id;
  $.post(
    "system/box/show_wall_report.php",
    {
      wall_report: id,
    },
    function (response) {
...
showNewsReport = function (id, item) {
  var post_id = id;
  $.post(
    "system/box/show_news_report.php",
    {
      news_report: id,
    },
    function (response) {
...
acceptWarn = function (t, id, p) {
  $.post(
    "system/action/action_member.php",
    {
      accept_warn: 1,
    },
    function (response) {
...
unsetReport = function (id, type) {
  hideOver();
  $.post(
    "system/action/action_reports.php",
    {
      unset_report: id,
      type: type,
    },
...
makeReport = function (t, p) {
  ...
  if (r == 0) {
    ...
    hideOver();
    $.post(
      "system/action/action_report.php",
      {
        send_report: 1,
        type: t,
        report: p,
...
openAddons = function () {
  ...
  showModal(
    '<div class="pad_box">' + addonsContent + '<div class="clear"></div></div>'
  );
};
getMonitor = function () {
  $("#monitor_data").toggle();
...
$(document).ready(function () {
  ...
  $(document).on(
    "click",
    ".other_panels, .addon_button, .head_li, #content",
    function () {
  ...
  $(document).on("click", ".post_video_save", function () {
    ...
    $(this)
      .removeClass("post_video_save")
      .addClass("post_video")
      .html('<video preload="auto" src="' + vlink + '" controls></video>');
  });

  $(document).on("click", ".post_audio_save", function () {
    var vlink = $(this).attr("data");
    $(this)
      .removeClass("post_audio_save")
      .addClass("post_audio")
      .html('<audio autoplay src="' + vlink + '" controls></audio>');
  });
  ...
  $(document).on(
    "click",
    "#news_file, #wall_file, #news_data, #friend_post",
    function () {
      hidePostEmoticon();
    }
  );
  ...
  searchUser = function () {
    ...
    boomDelay(function () {
      $.post(
        "system/action/action_search.php",
        {
